...
var SHOW_TOTALS_PER_PROJECT = true;
...
convertEntriesToCSV = function(timeEntries){
  var projectTimeTotals = getProjectTimeTotals(timeEntries);
  var csv = CSV_HEADERS + ',' + generateCSVStringFromKeys(projectTimeTotals) + '\n';
  for(var i = 0, j = timeEntries.length; i < j; i++)
  {
    if(i === 0)
      csv += createCSVEntryString(timeEntries[i]) + ',' + millisecondsToHMS(getTotalTime(timeEntries)) + ',' + generateCSVStringFromValues(projectTimeTotals, millisecondsToHMS) + '\n';
    else
...
generateCSVStringFromKeys = function(dict){
  var csvString = '';
  for(var key in dict){
    csvString += key + ',';
  };
...
generateCSVStringFromValues = function(dict, modifier){
  var csvString = '';
  for(var key in dict){
...
getProjectTimeTotals = function(timeEntries){
  if(SHOW_TOTALS_PER_PROJECT)
  {
    var projectsDict = {};
    timeEntries.forEach(function(entry){
      if(!(entry.project in projectsDict))
        projectsDict[entry.project] = 0;
      projectsDict[entry.project] += entry.dur;
    });
    return projectsDict;
  }
  else
